(function(){
  ...
  function firstPhaseCard(m,pair,names,isNext){return`<div style="border:1px solid ${isNext?'#efd596':'#c6e4d4'};border-radius:14px;padding:13px;background:${isNext?'#fffaf0':'#f3faf6'}"><div style="display:flex;justify-content:space-between;gap:10px"><strong>Partit ${safe(m.id)}</strong><span class="badge ${isNext?'orange':'green'}">${isNext?'Pròxim':'Confirmat'}</span></div><div style="font-size:18px;font-weight:950;color:#173d30;margin-top:7px">${safe(m.scheduled_at||'Horari pendent')}</div><div style="font-weight:850;color:#173d30;margin-top:5px">Rival: ${safe(other(m,pair.id,names))}</div><div class="meta" style="margin-top:5px">Pista 1 · Partit de 20 minuts</div></div>`}
  window.renderPlayer=function(player,d){
    ...
    if(mainPanel){const hospitality=mainPanel.querySelector('[style*="margin-top:16px"]'),firstPhase=d.matches.filter(m=>m.status!=='final'&&/^[A-H][1-3]$/.test(m.id)&&pairIn(m,pair.id)).sort((a,b)=>stamp(a)-stamp(b));if(firstPhase.length){const groupSection=document.createElement('div');groupSection.style.marginTop='18px';groupSection.innerHTML=`<div class="eyebrow">Primera fase</div><h3>Els teus 2 partits confirmats</h3><div style="display:grid;gap:9px">${firstPhase.map(m=>firstPhaseCard(m,pair,names,m===next)).join('')}</div><div class="meta" style="margin-top:9px">Els rivals i els horaris d’aquesta fase ja són definitius.</div>`;mainPanel.insertBefore(groupSection,hospitality||null)}const future=possible.filter(m=>!/^[A-H][1-3]$/.test(m.id));const section=document.createElement('div');section.style.marginTop='18px';section.innerHTML=`<div class="eyebrow">Camí del torneig</div><h3>Possibles partits següents</h3><div style="display:grid;gap:9px">${future.length?future.map(m=>card(m,pair,names)).join(''):'<div class="empty">Encara no hi ha cap altre partit possible per mostrar.</div>'}</div><div class="meta" style="margin-top:9px">S’actualitzen automàticament quan es resolen els grups i les eliminatòries.</div>`;mainPanel.insertBefore(section,hospitality||null)}
