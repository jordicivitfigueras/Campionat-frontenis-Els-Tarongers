(function(){
  ...
  function card(m,pairId,names){const confirmed=pairIn(m,pairId),rival=confirmed?other(m,pairId,names):'Depèn de la classificació';return`<div style="border:1px solid #dfe8e3;border-radius:14px;padding:13px;background:#f8faf9"><div style="display:flex;justify-content:space-between;gap:10px"><strong>${m.id==='FINAL'?'Final':'Partit '+safe(m.id)}</strong><span class="badge ${confirmed?'green':''}">${confirmed?'Confirmat':'Possible'}</span></div><div class="meta" style="margin-top:5px">${safe(m.stage||'')} · ${safe(m.scheduled_at||'Horari pendent')}</div><div style="font-weight:850;color:#173d30;margin-top:5px">Rival: ${safe(rival)}</div></div>`}
  window.renderPlayer=function(player,d){
    ...
    const confirmed=d.matches.filter(m=>m.status!=='final'&&pairIn(m,pair.id)).sort((a,b)=>stamp(a)-stamp(b)),next=confirmed[0],nextBox=box.querySelector('.next-match');
    if(nextBox&&next)nextBox.innerHTML=`<div class="stage">${safe(next.stage||next.id)} · ${safe(next.id)}</div><div class="when">${safe(next.scheduled_at||'Horari pendent')}</div><div style="font-weight:950;color:#173d30;margin-top:7px">Rival: ${safe(other(next,pair.id,names))}</div><div class="meta" style="margin-top:5px">Pista 1</div>`;
    const possible=possibleRows(pair,d,confirmed,next),mainPanel=box.querySelector('.main-grid .panel');
    if(mainPanel){const section=document.createElement('div');section.style.marginTop='18px';section.innerHTML=`<div class="eyebrow">Camí del torneig</div><h3>Possibles partits següents</h3><div style="display:grid;gap:9px">${possible.length?possible.map(m=>card(m,pair.id,names)).join(''):'<div class="empty">Encara no hi ha cap altre partit possible per mostrar.</div>'}</div><div class="meta" style="margin-top:9px">S’actualitzen automàticament quan es resolen els grups i les eliminatòries.</div>`;const hospitality=mainPanel.querySelector('[style*="margin-top:16px"]');mainPanel.insertBefore(section,hospitality||null)}
